...
class gsuiKeys extends gsui0ne {
	...
	static $keyNotation( id ) {
		$body.$css( "--gsuiKeys-keyNotation", `"${ gsuiKeys.$keyNotations[ id ][ 0 ] }"` );
	}
	...
	#setRootOctave( oct ) {
		this.#rootOctave = oct;
		this.$this.$query( ".gsuiKeys-key[data-root]" ).$rmAttr( "data-root" );
		this.$this.$query( `.gsuiKeys-key[data-midi="${ oct * 12 }"]` ).$addAttr( "data-root" );
	}
