...
class gsuiWindow extends gsui0ne {
	...
	constructor() {
		super( {
			$tagName: "gsui-window",
			$elements: {
				$icon: ".gsuiWindow-head > :first-child gsui-icon",
	...
	#onptrdownHead( e ) {
		...
		if ( clickHead && $.$isDblClick( e ) ) {
			...
			this.$elements.$head.$setPtrCapture( e.pointerId );
		}
	}
	#onptrdownHandlers( e ) {
		const tar = $( e.target );
		const dir = tar.$getAttr( "data-dir" );
	...
	#onptrmoveHead( e ) {
		...
		if ( !this.$this.$parent().$hasAttr( "lowgraphics" ) ) {
	...
	#onptrupHead( e ) {
		...
		this.$elements.$head.$relPtrCapture( e.pointerId );
		this.$this.$off( "pointermove", "pointerup" ).$rmAttr( "dragging" );
