...
class gsuiTimeline2 extends HTMLElement {
	constructor() {
		const children = GSUI.getTemplate( "gsui-timeline2" ),
	...
	_changePxPerBeat( ppb ) {
		...
		this.style.setProperty( "--gsuiTimeline2-beats-incr", onlyM ? this.beatsPerMeasure : 1 );
		this.style.setProperty( "--gsuiTimeline2-beats-opacity", beatsOpa );
	...
	_setStatus( st ) {
		this.classList.remove( `gsuiTimeline2-${ this._status }` );
	...
	_updateOffset() {
		...
		if ( diff ) {
			this._offset = off;
			this.style.setProperty( "--gsuiTimeline2-beats-offset", off );
		}
		return diff;
	}
	_updateNumberBeats() {
	...
	_updateMeasures() {
		Array.prototype.forEach.call( this._elBeats.children, ( el, i ) => {
			el.classList.toggle( "gsuiTimeline2-measure",
				this._onlyMeasures || ( this._offset + i ) % this.beatsPerMeasure === 0 );
		} );
	}
	_updateLoop() {
		this.classList.toggle( "gsuiTimeline2-looping", this.looping );
		if ( this.looping ) {
	...
	_onmousedown( e ) {
		const loopLine = e.target.classList.contains( "gsuiTimeline2-loopLine" );
		...
		if ( loopLine && Date.now() - this._mousedownDate > 500 ) {
			...
			this._setStatus(
				...
				e.target.classList.contains( "gsuiTimeline2-loopBody" ) ? "draggingLoopBody" :
				e.target.classList.contains( "gsuiTimeline2-loopHandleA" ) ? "draggingLoopHandleA" :
				e.target.classList.contains( "gsuiTimeline2-loopHandleB" ) || loopLine ? "draggingLoopHandleB" : "" );
			if ( this._status ) {
...
customElements.define( "gsui-timeline2", gsuiTimeline2 );
