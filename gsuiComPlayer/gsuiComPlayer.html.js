"use strict";

GSUsetTemplate( "gsui-com-player", () => [
	GSUcreateElement( "audio", { loop: true } ),
	GSUcreateButton( { class: "gsuiComPlayer-btn gsuiComPlayer-play", icon: "play" } ),
	GSUcreateDiv( { class: "gsuiComPlayer-body" },
		GSUcreateDiv( { class: "gsuiComPlayer-text" },
			GSUcreateDiv( { class: "gsuiComPlayer-name" },
				GSUcreateIcon( { icon: "private", inert: false, title: "Private" } ),
				GSUcreateIcon( { icon: "opensource", inert: false, title: "Open-source" } ),
				GSUcreateA( { class: "gsuiComPlayer-nameLink", href: false } ),
			),
			GSUcreateDiv( { class: "gsuiComPlayer-info-wrap" },
				GSUcreateDiv( { class: "gsuiComPlayer-info" },
					GSUcreateDiv( { class: "gsuiComPlayer-time" },
						GSUcreateIcon( { icon: "clock" } ),
						GSUcreateSpan( { class: "gsuiComPlayer-currentTime" } ),
						GSUcreateSpan( { class: "gsuiComPlayer-duration" } ),
					),
					GSUcreateDiv( { class: "gsuiComPlayer-tempo" },
						GSUcreateIcon( { icon: "speed" } ),
						GSUcreateSpan( { class: "gsuiComPlayer-bpm" } ),
					),
				),
			),
		),
		GSUcreateDiv( { class: "gsuiComPlayer-slider" },
			GSUcreateDiv(),
		),
	),
	GSUcreateAExt( { class: "gsuiComPlayer-btn gsuiComPlayer-dawlink gsuiIcon", "data-icon": "cu-music-spark", title: "Open in the DAW", href: false } ),
	GSUcreateButton( { class: "gsuiComPlayer-btn gsuiComPlayer-like" },
		GSUcreateIcon( { icon: "cu-heart-stroke" } ),
		GSUcreateIcon( { icon: "heart" } ),
		GSUcreateSpan(),
	),
	GSUcreateButton( { class: "gsuiComPlayer-btn gsuiComPlayer-actions", icon: "ellipsis-v" } ),
] );
